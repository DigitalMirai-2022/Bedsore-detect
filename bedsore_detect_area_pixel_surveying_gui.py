...
class BedsoresDetectionApp:
    ...
    def measure(self):
        ...
        # --- ここから計測処理 ---
        # 【変更①】キャンバス上の八角形の頂点（表示座標）を元画像のピクセル座標に変換する
        ...
        # 【変更②】シューライスの公式を利用して、ドラッグした八角形の面積をピクセル単位で計算
        area_pixels = polygon_area(octagon_points)

        # ※ここから実寸換算する場合の処理（任意）
        orig_img_width = self.original_cv_img.shape[1]
        pixel_size = SENSOR_WIDTH / orig_img_width  # 1ピクセルあたりのセンサー上の長さ (mm)
        conversion_factor = (pixel_size * distance) / FOCAL_LENGTH  # 物体面での1ピクセルの長さ（mm）
        real_area = area_pixels * (conversion_factor ** 2)  # mm²単位の実面積

        # 結果を表示（ピクセル面積と実寸面積の両方を表示）
        result_text = (f"八角形の面積: {area_pixels:.2f} px²\n"
                       f"実寸換算した面積: {real_area:.2f} mm²")
